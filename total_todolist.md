...
- [ ] Release blockers before public Android/server sync: replace dev/MVP payload `userId` device registration with authenticated user/session identity, decide who may register/re-register/revoke devices, implement the approved production endpoint source/config path, and decide Android Play signing/publishing requirements.
