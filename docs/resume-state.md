...
- Privacy boundary: screenshots are local developer artifacts only, captured
  inside Woong Monitor UI. No other-app content, typed text, passwords, form
...
- Android device-token persistence/client registration contracts are in place,
  Settings now exposes Register/Repair device, and Android upload calls attach
  `X-Device-Token`; secure production token storage, auth repair prompting,
  token refresh, and production endpoint policy remain open.
...
- Latest Android UI snapshot evidence:
  ...
  empty.
- Server auth/device registration is now paired with Android registration UI
  and upload token headers; secure token storage, auth repair prompting, token
  refresh, and production endpoint policy remain open before release use.
...
- The plan treats ?ÑÎèÑ/Í≤ΩÎèÑ as sensitive metadata: off by default, Android
...
quality gap is covered, and app/domain chart `?ÅÏÑ∏Î≥¥Í∏∞` actions now switch to
...
- Added app/domain chart `?ÅÏÑ∏Î≥¥Í∏∞` buttons in `Views/ChartsPanel.xaml`.
...
- Replaced the `ChartsPanel` outer card `Border` with `controls:SectionCard`
  ...
  AutomationIds, and app/domain `?ÅÏÑ∏Î≥¥Í∏∞` tab-switch command behavior.
...
- `ªÛºº∫∏±‚` now goes through `IDashboardChartDetailsPresenter` and opens a dedicated WPF detail window with up to 10 horizontal bars.
...
- Updated the domain focus chart header so `µµ∏ﬁ¿Œ∫∞` remains on the first line and `¡˝¡ﬂ Ω√∞£` wraps to the next line.
- Kept the `ªÛºº∫∏±‚` action button in the existing right-side header position.
...
- Fixed the user-reported Current Focus problem: Dashboard now prefers the latest meaningful tracked external Room session, such as `Chrome / com.android.chrome`, instead of showing the Monitor app process itself or low-value launcher rows.
...
- Updated the Dashboard route so MainActivity no longer injects its own package name as the current foreground app.
- Dashboard Current Focus now selects the latest meaningful tracked external app from Room, skipping `com.woong.monitorstack` and launcher noise when a real usage session such as Chrome is available.
- Added RED/GREEN Robolectric coverage for returning from Chrome while Monitor is foreground: the Current Focus panel must show `Chrome / com.android.chrome`.
...
- Worker I strengthened the Android app-switch/current-focus QA lane only.
- Added instrumentation test `dashboardAfterChromeReturnShowsWoongAsCurrentAndChromeAsLatestExternal` in `AppSwitchQaEvidenceTest`. It launches the real `MainActivity` after UsageStats collection, verifies Dashboard Current Focus shows `Woong Monitor / com.woong.monitorstack` as current, verifies `Chrome / com.android.chrome` remains the latest collected external app, and writes metadata-only `dashboard-current-focus-evidence.json` with matching Room focus-session counts.
- Updated `scripts/run-android-app-switch-qa.ps1` to execute that test, pull `dashboard-current-focus-evidence.json`, `dashboard-current-focus-after-chrome-return.png`, and `dashboard-current-focus-after-chrome-return.xml`, and fail the QA run if the current/latest-external packages are wrong.
- Privacy boundary preserved: no Chrome screenshot, Chrome UI hierarchy, browser/page contents, typed text, form contents, clipboard data, passwords, messages, or global touch coordinates are collected. Chrome evidence remains package/time metadata from UsageStats/Room plus foreground/process text metadata from the script.
- Connected emulator evidence is currently BLOCKED: `adb devices -l` reported no attached device/emulator. No-device app-switch script validation produced `artifacts/android-app-switch-qa/20260503-195857/` with BLOCKED `dashboard-current-focus-evidence.json`.
- Validation passed: `./gradlew.bat :app:assembleDebugAndroidTest --no-daemon --stacktrace`; `./gradlew.bat :app:testDebugUnitTest --tests com.woong.monitorstack.dashboard.DashboardCurrentFocusResolverTest --tests com.woong.monitorstack.usage.UsageSessionizerTest --no-daemon --stacktrace --console=plain`; `./gradlew.bat :app:assembleDebug :app:assembleDebugAndroidTest --no-daemon --stacktrace --console=plain`; `./gradlew.bat :app:testDebugUnitTest --no-daemon --stacktrace --console=plain`; `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-android-app-switch-qa.ps1 -SkipBuild`.

## 2026-05-03 Android UI Snapshot Acceptance Trustworthiness

- Worker H hardened Android UI screenshot acceptance after the bottom-nav fix.
- Added RED/GREEN architecture coverage: `AndroidUiSnapshotReportValidator_RejectsPassReportWithoutHierarchyEvidence` failed before the validator change because a PASS report with real PNG files but no UI hierarchy XML was accepted, then passed after the hierarchy gate.
- `scripts/validate-android-ui-snapshot-report.ps1` now requires accepted PNGs to be parseable/nonblank, parses UIAutomator XML, checks Splash branding, permission privacy guidance, Dashboard Current Focus IDs/text, Dashboard location card/map IDs/text, Report and Settings tab IDs/text, and all Dashboard/Sessions/Report/Settings bottom-nav tab labels.
- The report validator also invokes `validate-android-bottom-nav-floor.ps1` for the Dashboard/Sessions/Report/Settings hierarchy files so standalone report validation catches both blank-floor and missing-tab evidence.
- `scripts/run-android-ui-snapshots.ps1` now writes the expected hierarchy and pixel gates into `report.md` and `manifest.json`; no-device runs remain `BLOCKED` and include a next action instead of stale PASS/FAIL evidence.
- `SnapshotCaptureTest` now asserts each source screenshot and hierarchy dump exists and is non-empty before the script pulls artifacts.
- Connected emulator evidence is currently BLOCKED: `adb devices -l` reported no attached device/emulator. Fresh no-device snapshot artifacts were written to `artifacts/android-ui-snapshots/20260503-195857/`.
- Validation passed: `dotnet test tests\Woong.MonitorStack.Architecture.Tests\Woong.MonitorStack.Architecture.Tests.csproj --no-restore --filter "FullyQualifiedName~AndroidUiSnapshot" -maxcpucount:1 -v minimal` (13 passed); `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\run-android-ui-snapshots.ps1 -SkipBuild` (BLOCKED, no device); `.\gradlew.bat :app:testDebugUnitTest --no-daemon --max-workers=1 --stacktrace`; and `.\gradlew.bat :app:assembleDebug :app:assembleDebugAndroidTest --no-daemon --max-workers=1 --stacktrace`.
- Main integration updated the fake-ADB architecture fixtures for the new current-focus evidence contract, reran focused/full .NET and Android validation, and will commit/push this slice from the main worktree.

## 2026-05-03 WPF Sync/Bridge/Outbox Hardening Batch

- Integrated subagent work for protected WPF sync token storage, WPF UI test helper consolidation, bounded Android Room DB refresh during local dashboard bridge loops, focus-session duplicate outbox prevention, and sync_outbox aggregate idempotency/terminal-state safety.
- Added IWindowsSyncTokenStore, file-backed protected token persistence, and current-user DPAPI protection groundwork. Configured environment tokens remain a development/config bridge and are not displayed in Settings, written to SQLite schema, or auto-persisted to the token store.
- run-local-integrated-dashboard.ps1 now supports explicit bounded -RefreshAndroidDbEachBridgeIteration, pulling the Android emulator Room DB before each one-shot bridge upload iteration while preserving the default one-shot and bridge-internal interval behaviors.
- WPF App UI expectation tests now use shared WpfTestHelpers for visual descendant and AutomationId lookup, with architecture guards preventing bespoke traversal helper drift.
- Windows local storage now prevents duplicate focus-session saves from enqueuing duplicate outbox rows, adds unique sync_outbox aggregate identity, dedupes legacy aggregate rows, and keeps already-synced rows terminal when late failure/sync callbacks arrive.
- Read-only follow-up plans were recorded for browser native-message ingestion: durable raw-event identity plus atomic raw event + web-session + outbox persistence, and for local dashboard live/current-app state: metadata-only current-state upload contracts rather than deriving live state only from the latest completed focus session.
- Validation passed: focused LocalIntegratedDashboard runbook tests (8), focused WPF App helper/sync composition tests (61), focused Windows token tests (11), focused Windows storage/sync tests (44), full dotnet test Woong.MonitorStack.sln --no-restore -maxcpucount:1 -v minimal (743 passed, 6 PostgreSQL-environment skips), full dotnet build Woong.MonitorStack.sln --no-restore -maxcpucount:1 -v minimal (0 warnings/errors), clean coverage generation (line 87.7%, branch 70.4%), ReportGenerator output under artifacts/coverage, and Windows smoke foreground metadata capture.

## 2026-05-03 Browser Raw Event And Current-App Server Foundation

- Added durable browser raw-event identity through clientEventId, Chrome extension message emission, metadata-only fallback id derivation, browser_raw_event legacy backfill, unique client_event_id index, and duplicate-safe local raw-event persistence.
- Added metadata-only current-app state upload contracts and server foundation: current_app_states table/entity, upload endpoint/service, EF migration 20260503130736_AddCurrentAppStateTable, dashboard preference for current state over latest completed focus session, and production migration runbook coverage.
- Privacy boundaries remain metadata-only: current app state and browser raw-event ids do not include typed text, page contents, screenshots, clipboard, passwords, form input, or Android global touch coordinates.
